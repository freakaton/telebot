...
def search_movies(message):
    ...
    # if 1 or more movie has been found
        ...
        def choiceFromList(message):
            ...
            if text.isdigit():
                ...
                # If in list..
                    ...
                    msg = bot.send_message(message.chat.id, fullDescOfMovie(getMovieById(movie['id'], MOVIEDB_TOKEN)),
                                           parse_mode="HTML")
                    bot.register_next_step_handler(msg, search_movies)
                    return True
        ...
        if result:
            ...
            bot.send_photo(message.chat.id, image)
            msg = bot.send_message(message.chat.id, fullDescOfMovie(getMovieById(result['id'], MOVIEDB_TOKEN)),
                                   parse_mode="HTML")
            bot.register_next_step_handler(msg, search_movies)
            return True
        else:
